...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (checkIn && checkOut) {
    ...
    if (startDate.isValid() && endDate.isValid()) {
      ...
      $("#daterange").val(startDate.format("DD-MMM-YYYY") + " - " + endDate.format("DD-MMM-YYYY"));
  ...
  function toggleCrashpadSelection(crashpadId) {
    const card = document.querySelector(`.crashpad-card[data-crashpad-id="${crashpadId}"]`);
      ...
      selectedCrashpads.forEach((crashpadId) => {
        ...
        if (days >= 14) {
          dailyRate = parseFloat(
            crashpadCard.querySelector(".price td:nth-child(3)").textContent.replace("€", "")
          );
        } else if (days >= 7) {
          dailyRate = parseFloat(
            crashpadCard.querySelector(".price td:nth-child(2)").textContent.replace("€", "")
          );
        } else {
          dailyRate = parseFloat(
            crashpadCard.querySelector(".price td:first-child").textContent.replace("€", "")
          );
        }
  ...
  if (addToCartBtn) {
    addToCartBtn.addEventListener("click", async (e) => {
      ...
      if (!dateRange.startDate || !dateRange.endDate || selectedCrashpads.size === 0) {
        showToast({
          title: "Error",
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data.error || `Server returned ${response.status}: ${JSON.stringify(data)}`
          );
        }
